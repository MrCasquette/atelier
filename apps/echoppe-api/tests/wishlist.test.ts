...
describe('B7 — wishlist', () => {
  ...
  it('POST épingle la variante, GET la liste enrichie', async () => {
    ...
    const list = records(await (await req('GET', '/wishlist', { cookie })).json());
    expect(list).toHaveLength(1);
    expect(record(list[0]?.variant, 'variant').id).toBe(variantId);
    expect(record(list[0]?.variant, 'variant').priceHt).toBe('19.90');
    expect(record(list[0]?.product, 'product').slug).toBe('wish-prod');
  });
