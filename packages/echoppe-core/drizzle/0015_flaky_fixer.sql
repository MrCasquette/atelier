...
--
-- On GARANTIT d'abord la cible. Le seed crée `admin`, mais une installation où il n'a jamais tourné
-- n'a que les rôles qu'on lui a faits — l'instance de démonstration n'avait que « Propriétaire ».
-- Sans cette ligne, la réassignation serait un no-op silencieux et laisserait des utilisateurs assis
-- sur un rôle vidé de ses permissions, que le code ne connaît plus.
INSERT INTO "role" ("key", "name", "description", "scope", "is_system")
SELECT 'admin', 'Administrateur', 'Administrateur — tout, hors gouvernance sensible',
       'admin'::role_scope, true
WHERE EXISTS (SELECT 1 FROM "role" WHERE "key" = 'owner')
  AND NOT EXISTS (SELECT 1 FROM "role" WHERE "key" = 'admin');--> statement-breakpoint
...
UPDATE "user" SET "role" = (SELECT "id" FROM "role" WHERE "key" = 'admin')
  WHERE "role" IN (SELECT "id" FROM "role" WHERE "key" = 'owner');--> statement-breakpoint
