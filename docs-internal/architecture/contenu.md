...
`@axiome-apps/atelier-content` (P2). La doc utilisateur (le _comment_) vit dans
...
| Le framework | Le dev |
|--------------|--------|
| **Stocke** les blocs (`page` → `section[]`, `data` en jsonb) | **Déclare** ses blocs en config-as-code (`@axiome-apps/atelier-content`) |
...
## `@axiome-apps/atelier-content` : léger par design, mais pas dogmatique
